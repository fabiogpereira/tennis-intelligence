...
## Current phase
Phase 1 is complete and Phase 2 MCP data-foundation work is active. Research #01 is Tennis DNA; the original pressure work is preserved as Research #02. Parser v0.2, serve reconciliation, and the aggregate stability pilot retain serve-only candidates for context-controlled falsification, not publication. The automated ATP/WTA context join remains unapproved until its deterministic sample is human-reviewed. Do not implement a Tennis DNA vector, similarity model, PPI, or the final application until feature-specific coverage, context, player-level uncertainty, and stability questions have been reviewed. Record significant disagreements and methodology changes in the relevant document or an ADR.

## How to run checks
The repository currently has no third-party runtime dependencies. From the repository root, run the narrow checks with `python -m unittest discover -s tests -v`, regenerate feasibility reports with `python -m research.experiments.profile_mcp_snapshot`, reproduce the serve pilot with `python -m research.experiments.serve_stability`, audit context identity with `python -m research.experiments.audit_context_join`, then perform Markdown link/path review and `git diff --check` when the directory is inside a Git worktree.
